                                                                              ...
                                                                              const ManagedColorDataCreateSource    &source)
{
    ...
    for (ModelVolume *volume : object.volumes) {
        ...
        volume_source.background = rgb_metadata_background_color(*rgba_data);
...
public:
        ...
        , m_initial_filament_count(filament_colors.size())
    {
        ...
        const int slot_limit = std::max(1, int(static_cast<size_t>(EnforcerBlockerType::ExtruderMax)));
...
private:
    ...
    {
        count = std::clamp<size_t>(count, 1, static_cast<size_t>(EnforcerBlockerType::ExtruderMax));
...
static bool region_color_apply_filament_colors(const std::vector<ColorRGBA> &filament_colors, size_t active_filament_count)
{
    ...
    const size_t target_count = std::min(std::max(previous_count, active_filament_count), static_cast<size_t>(EnforcerBlockerType::ExtruderMax));
    if (target_count > previous_count) {
        ...
        if (wxGetApp().plater() != nullptr)
            wxGetApp().plater()->on_filament_count_change(target_count);
...
static bool convert_object_to_color_regions(ModelObject &object, const ManagedColorDataCreateSource &source, wxWindow *parent)
{
    ...
    const size_t max_physical_filaments = std::min(filament_colors.size(), static_cast<size_t>(EnforcerBlockerType::ExtruderMax));
