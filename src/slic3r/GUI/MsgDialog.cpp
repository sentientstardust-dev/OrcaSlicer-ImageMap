...
void MsgDialog::SetButtonStyle(wxWindowID btn_id, ButtonStyle style, ButtonType type)
{
    if (Button* btn = get_button(btn_id))
        btn->SetStyle(style, type);
}
...
void MsgDialog::apply_style(long style)
{
    ...
    logo->SetBitmap( create_scaled_bitmap(style & wxAPPLY        ? "completed" :
                                          ...
                                          style & wxICON_QUESTION       ? "question"    : "OrcaSlicer_128px.png", this, 64, style & wxICON_ERROR));
}
